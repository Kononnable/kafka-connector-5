...
impl ConnectionPool {
    ...
    pub fn send_api_request(
        ...
        if let Some(conn) = self.connections.iter_mut().find(|c| c.node_id() == broker_id) {
        ...
        for addr in addrs {
            match TcpStream::connect(*addr) {
                Ok(mut stream) => {
                    ...
                    if registry.register(&mut stream, token, mio::Interest::READABLE | mio::Interest::WRITABLE).is_ok() {
                        let mut conn = Connection::new(token, stream, self.client_id.clone(), broker_id);
                        conn.send_raw_request(payload);
                        self.connections.push(conn);
                        return Ok(());
                    }
