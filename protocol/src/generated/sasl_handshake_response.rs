...
impl ApiResponse for SaslHandshakeResponse {
    ...
    fn serialize(&self, version: crate::traits::ApiVersion, buf: &mut BytesMut) -> Result<(), SerializationError> {
        ...
        self.mechanisms.encode_flexible(buf, is_flexible).map_err(|_| SerializationError::Encode("failed to encode Mechanisms"))?;
        if is_flexible {
    ...
    fn deserialize(version: crate::traits::ApiVersion, buf: &mut Bytes) -> Result<Self, SerializationError> {
        ...
        let mechanisms = <Vec<String> as KafkaDeserialize>::decode_flexible(buf, is_flexible).map_err(|_| SerializationError::Decode("failed to decode Mechanisms"))?;
        Ok(Self { error_code, mechanisms })
    }
}
impl KafkaSerialize for SaslHandshakeResponse {
    fn encode<B: BufMut>(&self, buf: &mut B) -> Result<(), EncodeError> {
        ...
        self.mechanisms.encode(buf).map_err(|_| EncodeError::ValueTooLarge { message: "failed to encode Mechanisms".into() })?;
        Ok(())
    }
    fn encode_flexible<B: BufMut>(&self, buf: &mut B, is_flexible: bool) -> Result<(), EncodeError> {
        ...
        self.mechanisms.encode_flexible(buf, is_flexible).map_err(|_| EncodeError::ValueTooLarge { message: "failed to encode Mechanisms".into() })?;
        if is_flexible {
            // Tagged fields (none yet)
            crate::protocol::serialization::encode_unsigned_varint(0u64, buf);
        }
...
impl KafkaDeserialize for SaslHandshakeResponse {
    fn decode<B: Buf>(buf: &mut B) -> Result<Self, DecodeError> {
        ...
        let mechanisms = <Vec<String> as KafkaDeserialize>::decode(buf).map_err(|_| DecodeError::Protocol { message: "failed to decode Mechanisms".into() })?;
        Ok(Self { error_code, mechanisms })
    }
    fn decode_flexible<B: Buf>(buf: &mut B, is_flexible: bool) -> Result<Self, DecodeError> {
        ...
        let mechanisms = <Vec<String> as KafkaDeserialize>::decode_flexible(buf, is_flexible).map_err(|_| DecodeError::Protocol { message: "failed to decode Mechanisms".into() })?;
        if is_flexible {
            // Tagged fields (skip)
