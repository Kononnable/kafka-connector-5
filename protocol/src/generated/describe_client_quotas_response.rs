...
impl ApiResponse for DescribeClientQuotasResponse {
    ...
    fn deserialize(version: crate::traits::ApiVersion, buf: &mut Bytes) -> Result<Self, SerializationError> {
        ...
        let entries = <Option<Vec<EntryData>> as KafkaDeserialize>::decode_flexible(buf, is_flexible).map_err(|_| SerializationError::Decode("failed to decode Entries"))?;
        Ok(Self { throttle_time_ms, error_code, error_message, entries })
    }
}
    ...
    fn encode_flexible<B: BufMut>(&self, buf: &mut B, is_flexible: bool) -> Result<(), EncodeError> {
        ...
        if is_flexible {
            if let Some(ref __val) = self.entries {
                crate::protocol::serialization::encode_unsigned_varint(1u64, buf);
                __val.encode_flexible(buf, true).map_err(|_| EncodeError::ValueTooLarge { message: "failed to encode Entries".into() })?;
            } else {
                crate::protocol::serialization::encode_unsigned_varint(0u64, buf);
            }
        } else {
            if let Some(ref __val) = self.entries {
                __val.encode(buf).map_err(|_| EncodeError::ValueTooLarge { message: "failed to encode Entries".into() })?;
            }
        }
        if is_flexible {
            // Tagged fields (none yet)
...
impl KafkaDeserialize for DescribeClientQuotasResponse {
    fn decode<B: Buf>(buf: &mut B) -> Result<Self, DecodeError> {
        ...
        let entries = <Option<Vec<EntryData>> as KafkaDeserialize>::decode(buf).map_err(|_| DecodeError::Protocol { message: "failed to decode Entries".into() })?;
        Ok(Self { throttle_time_ms, error_code, error_message, entries })
    }
    fn decode_flexible<B: Buf>(buf: &mut B, is_flexible: bool) -> Result<Self, DecodeError> {
        ...
        let entries = if is_flexible {
            ...
            if __present == 0 {
                ...
                Some(<Vec<EntryData> as KafkaDeserialize>::decode_flexible(buf, true).map_err(|_| DecodeError::Protocol { message: "failed to decode Entries".into() })?)
            }
        } else {
            <Option<Vec<EntryData>> as KafkaDeserialize>::decode(buf).map_err(|_| DecodeError::Protocol { message: "failed to decode Entries".into() })?
        };
        if is_flexible {
            // Tagged fields (skip)
...
impl KafkaSerialize for EntryData {
    fn encode<B: BufMut>(&self, buf: &mut B) -> Result<(), EncodeError> {
        ...
        self.values.encode(buf).map_err(|_| EncodeError::ValueTooLarge { message: "failed to encode Values".into() })?;
        Ok(())
    }
    fn encode_flexible<B: BufMut>(&self, buf: &mut B, is_flexible: bool) -> Result<(), EncodeError> {
        self.entity.encode_flexible(buf, is_flexible).map_err(|_| EncodeError::ValueTooLarge { message: "failed to encode Entity".into() })?;
        self.values.encode_flexible(buf, is_flexible).map_err(|_| EncodeError::ValueTooLarge { message: "failed to encode Values".into() })?;
        if is_flexible {
            // Tagged fields (none yet)
            crate::protocol::serialization::encode_unsigned_varint(0u64, buf);
        }
...
impl KafkaDeserialize for EntryData {
    fn decode<B: Buf>(buf: &mut B) -> Result<Self, DecodeError> {
        ...
        let values = <Vec<ValueData> as KafkaDeserialize>::decode(buf).map_err(|_| DecodeError::Protocol { message: "failed to decode Values".into() })?;
    ...
    fn decode_flexible<B: Buf>(buf: &mut B, is_flexible: bool) -> Result<Self, DecodeError> {
        ...
        let values = <Vec<ValueData> as KafkaDeserialize>::decode_flexible(buf, is_flexible).map_err(|_| DecodeError::Protocol { message: "failed to decode Values".into() })?;
        if is_flexible {
            // Tagged fields (skip)
            let (_tag_count, _) = crate::protocol::serialization::decode_unsigned_varint(buf)?;
        }
