    ...
    fn deserialize(
        ...
        let replica_state = if (15) <= version.0 {
            if is_flexible {
                ...
                <ReplicaState as KafkaDeserialize>::decode_flexible(buf, version, is_flexible)
...
impl KafkaDeserialize for FetchRequest {
    fn decode<B: Buf>(buf: &mut B) -> Result<Self, DecodeError> {
        ...
        tracing::trace!(
            "  [{}] classic decode field `ReplicaState` ({} bytes remaining)",
            stringify!(Self),
            buf.remaining()
        );
        let replica_state =
        ...
        tracing::trace!(
            "  [{}] classic decode field `IsolationLevel` ({} bytes remaining)",
            stringify!(Self),
            buf.remaining()
        );
        let isolation_level =
        ...
        tracing::trace!(
            "  [{}] classic decode field `SessionEpoch` ({} bytes remaining)",
            stringify!(Self),
            buf.remaining()
        );
        let session_epoch =
    ...
    fn decode_flexible<B: Buf>(
        ...
        tracing::trace!(
            "  [{}] decoding field `ReplicaState` ({} bytes remaining)",
            stringify!(Self),
            buf.remaining()
        );
        let replica_state = if (15) <= version.0 {
        ...
        tracing::trace!(
            "  [{}] decoding field `IsolationLevel` ({} bytes remaining)",
            stringify!(Self),
            buf.remaining()
        );
        let isolation_level = if (4) <= version.0 {
        ...
        tracing::trace!(
            "  [{}] decoding field `SessionEpoch` ({} bytes remaining)",
            stringify!(Self),
            buf.remaining()
        );
        let session_epoch = if (7) <= version.0 {
...
impl KafkaDeserialize for FetchPartition {
    fn decode<B: Buf>(buf: &mut B) -> Result<Self, DecodeError> {
        ...
        tracing::trace!(
            "  [{}] classic decode field `FetchOffset` ({} bytes remaining)",
            stringify!(Self),
            buf.remaining()
        );
        let fetch_offset =
        ...
        tracing::trace!(
            "  [{}] classic decode field `LastFetchedEpoch` ({} bytes remaining)",
            stringify!(Self),
            buf.remaining()
        );
    ...
    fn decode_flexible<B: Buf>(
        ...
        tracing::trace!(
            "  [{}] decoding field `FetchOffset` ({} bytes remaining)",
            stringify!(Self),
            buf.remaining()
        );
        let fetch_offset = <i64 as KafkaDeserialize>::decode_flexible(buf, version, is_flexible)
            .map_err(|_| DecodeError::Protocol {
                message: "failed to decode FetchOffset".into(),
            })?;
        tracing::trace!(
            "  [{}] decoding field `LastFetchedEpoch` ({} bytes remaining)",
            stringify!(Self),
            buf.remaining()
        );
