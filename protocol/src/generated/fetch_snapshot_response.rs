...
impl ApiResponse for FetchSnapshotResponse {
    ...
    fn deserialize(
        ...
        let topics =
            <Vec<TopicSnapshot> as KafkaDeserialize>::decode_flexible(buf, version, is_flexible)
...
impl KafkaDeserialize for NodeEndpoint {
    ...
    fn decode_flexible<B: Buf>(
        ...
        let port = if (1) <= version.0 {
            <u16 as KafkaDeserialize>::decode_flexible(buf, version, is_flexible).map_err(|_| {
                DecodeError::Protocol {
                    message: "failed to decode Port".into(),
                }
            })?
...
impl KafkaDeserialize for PartitionSnapshot {
    fn decode<B: Buf>(buf: &mut B) -> Result<Self, DecodeError> {
        ...
        tracing::trace!(
            "  [{}] classic decode field `Size` ({} bytes remaining)",
            stringify!(Self),
            buf.remaining()
        );
        ...
        tracing::trace!(
            "  [{}] classic decode field `Position` ({} bytes remaining)",
            stringify!(Self),
            buf.remaining()
        );
        let position =
        ...
        tracing::trace!(
            "  [{}] classic decode field `UnalignedRecords` ({} bytes remaining)",
            stringify!(Self),
            buf.remaining()
        );
    ...
    fn decode_flexible<B: Buf>(
        ...
        let index = <i32 as KafkaDeserialize>::decode_flexible(buf, version, is_flexible).map_err(
            |_| DecodeError::Protocol {
                message: "failed to decode Index".into(),
            },
        )?;
        ...
        let snapshot_id =
            <SnapshotId as KafkaDeserialize>::decode_flexible(buf, version, is_flexible).map_err(
                |_| DecodeError::Protocol {
                    message: "failed to decode SnapshotId".into(),
                },
            )?;
        ...
        tracing::trace!(
            "  [{}] decoding field `Size` ({} bytes remaining)",
            stringify!(Self),
            buf.remaining()
        );
        let size = <i64 as KafkaDeserialize>::decode_flexible(buf, version, is_flexible).map_err(
            |_| DecodeError::Protocol {
                message: "failed to decode Size".into(),
            },
        )?;
        tracing::trace!(
            "  [{}] decoding field `Position` ({} bytes remaining)",
            stringify!(Self),
            buf.remaining()
        );
        let position = <i64 as KafkaDeserialize>::decode_flexible(buf, version, is_flexible)
            .map_err(|_| DecodeError::Protocol {
                message: "failed to decode Position".into(),
            })?;
        tracing::trace!(
            "  [{}] decoding field `UnalignedRecords` ({} bytes remaining)",
            stringify!(Self),
            buf.remaining()
        );
        let unaligned_records =
...
impl KafkaDeserialize for TopicSnapshot {
    ...
    fn decode_flexible<B: Buf>(
        ...
        let partitions = <Vec<PartitionSnapshot> as KafkaDeserialize>::decode_flexible(
            buf,
            version,
            is_flexible,
        )
