...
impl ApiRequest for DescribeDelegationTokenRequest {
    ...
    fn serialize(&self, version: crate::traits::ApiVersion, buf: &mut BytesMut) -> Result<(), SerializationError> {
        ...
        self.owners.encode_flexible(buf, is_flexible).map_err(|_| SerializationError::Encode("failed to encode Owners"))?;
        if is_flexible {
    ...
    fn deserialize(version: crate::traits::ApiVersion, buf: &mut Bytes) -> Result<Self, SerializationError> {
        let is_flexible = (2) <= version.0;
        let owners = <Option<Vec<DescribeDelegationTokenOwner>> as KafkaDeserialize>::decode_flexible(buf, is_flexible).map_err(|_| SerializationError::Decode("failed to decode Owners"))?;
        Ok(Self { owners })
    }
}
impl KafkaSerialize for DescribeDelegationTokenRequest {
    fn encode<B: BufMut>(&self, buf: &mut B) -> Result<(), EncodeError> {
        self.owners.encode(buf).map_err(|_| EncodeError::ValueTooLarge { message: "failed to encode Owners".into() })?;
        Ok(())
    }
    fn encode_flexible<B: BufMut>(&self, buf: &mut B, is_flexible: bool) -> Result<(), EncodeError> {
        if is_flexible {
            if let Some(ref __val) = self.owners {
                ...
                __val.encode_flexible(buf, true).map_err(|_| EncodeError::ValueTooLarge { message: "failed to encode Owners".into() })?;
            } else {
                crate::protocol::serialization::encode_unsigned_varint(0u64, buf);
            }
        } else {
            if let Some(ref __val) = self.owners {
                __val.encode(buf).map_err(|_| EncodeError::ValueTooLarge { message: "failed to encode Owners".into() })?;
            }
        }
        if is_flexible {
            // Tagged fields (none yet)
...
impl KafkaDeserialize for DescribeDelegationTokenRequest {
    fn decode<B: Buf>(buf: &mut B) -> Result<Self, DecodeError> {
        let owners = <Option<Vec<DescribeDelegationTokenOwner>> as KafkaDeserialize>::decode(buf).map_err(|_| DecodeError::Protocol { message: "failed to decode Owners".into() })?;
        Ok(Self { owners })
    }
    fn decode_flexible<B: Buf>(buf: &mut B, is_flexible: bool) -> Result<Self, DecodeError> {
        let owners = if is_flexible {
            ...
            if __present == 0 {
                ...
                Some(<Vec<DescribeDelegationTokenOwner> as KafkaDeserialize>::decode_flexible(buf, true).map_err(|_| DecodeError::Protocol { message: "failed to decode Owners".into() })?)
            }
        } else {
            <Option<Vec<DescribeDelegationTokenOwner>> as KafkaDeserialize>::decode(buf).map_err(|_| DecodeError::Protocol { message: "failed to decode Owners".into() })?
        };
        if is_flexible {
            // Tagged fields (skip)
