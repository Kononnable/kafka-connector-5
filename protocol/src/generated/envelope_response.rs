...
impl ApiResponse for EnvelopeResponse {
    ...
    fn serialize(&self, version: crate::traits::ApiVersion, buf: &mut BytesMut) -> Result<(), SerializationError> {
        ...
        self.response_data.encode_flexible(buf, is_flexible).map_err(|_| SerializationError::Encode("failed to encode ResponseData"))?;
    ...
    fn deserialize(version: crate::traits::ApiVersion, buf: &mut Bytes) -> Result<Self, SerializationError> {
        let is_flexible = true;
        let response_data = <Option<Vec<u8>> as KafkaDeserialize>::decode_flexible(buf, is_flexible).map_err(|_| SerializationError::Decode("failed to decode ResponseData"))?;
        ...
        Ok(Self { response_data, error_code })
    }
}
impl KafkaSerialize for EnvelopeResponse {
    fn encode<B: BufMut>(&self, buf: &mut B) -> Result<(), EncodeError> {
        self.response_data.encode(buf).map_err(|_| EncodeError::ValueTooLarge { message: "failed to encode ResponseData".into() })?;
    ...
    fn encode_flexible<B: BufMut>(&self, buf: &mut B, is_flexible: bool) -> Result<(), EncodeError> {
        if is_flexible {
            if let Some(ref __val) = self.response_data {
                ...
                __val.encode_flexible(buf, true).map_err(|_| EncodeError::ValueTooLarge { message: "failed to encode ResponseData".into() })?;
            } else {
                crate::protocol::serialization::encode_unsigned_varint(0u64, buf);
            }
        } else {
            if let Some(ref __val) = self.response_data {
                __val.encode(buf).map_err(|_| EncodeError::ValueTooLarge { message: "failed to encode ResponseData".into() })?;
            }
        }
...
impl KafkaDeserialize for EnvelopeResponse {
    fn decode<B: Buf>(buf: &mut B) -> Result<Self, DecodeError> {
        let response_data = <Option<Vec<u8>> as KafkaDeserialize>::decode(buf).map_err(|_| DecodeError::Protocol { message: "failed to decode ResponseData".into() })?;
    ...
    fn decode_flexible<B: Buf>(buf: &mut B, is_flexible: bool) -> Result<Self, DecodeError> {
        let response_data = if is_flexible {
            ...
            if __present == 0 {
                ...
                Some(<Vec<u8> as KafkaDeserialize>::decode_flexible(buf, true).map_err(|_| DecodeError::Protocol { message: "failed to decode ResponseData".into() })?)
            }
        } else {
            <Option<Vec<u8>> as KafkaDeserialize>::decode(buf).map_err(|_| DecodeError::Protocol { message: "failed to decode ResponseData".into() })?
        };
