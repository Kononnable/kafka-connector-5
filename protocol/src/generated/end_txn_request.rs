...
impl ApiRequest for EndTxnRequest {
    ...
    fn get_api_key() -> ApiKey { ApiKey::new(26) }
    ...
    fn serialize(&self, version: crate::traits::ApiVersion, buf: &mut BytesMut) -> Result<(), SerializationError> {
        ...
        self.committed.encode_flexible(buf, is_flexible).map_err(|_| SerializationError::Encode("failed to encode Committed"))?;
        if is_flexible {
    ...
    fn deserialize(version: crate::traits::ApiVersion, buf: &mut Bytes) -> Result<Self, SerializationError> {
        ...
        let committed = <bool as KafkaDeserialize>::decode_flexible(buf, is_flexible).map_err(|_| SerializationError::Decode("failed to decode Committed"))?;
...
impl KafkaSerialize for EndTxnRequest {
    fn encode<B: BufMut>(&self, buf: &mut B) -> Result<(), EncodeError> {
        ...
        self.committed.encode(buf).map_err(|_| EncodeError::ValueTooLarge { message: "failed to encode Committed".into() })?;
        Ok(())
    }
    fn encode_flexible<B: BufMut>(&self, buf: &mut B, is_flexible: bool) -> Result<(), EncodeError> {
        ...
        self.committed.encode_flexible(buf, is_flexible).map_err(|_| EncodeError::ValueTooLarge { message: "failed to encode Committed".into() })?;
        if is_flexible {
            // Tagged fields (none yet)
            crate::protocol::serialization::encode_unsigned_varint(0u64, buf);
        }
...
impl KafkaDeserialize for EndTxnRequest {
    fn decode<B: Buf>(buf: &mut B) -> Result<Self, DecodeError> {
        ...
        let committed = <bool as KafkaDeserialize>::decode(buf).map_err(|_| DecodeError::Protocol { message: "failed to decode Committed".into() })?;
        Ok(Self { transactional_id, producer_id, producer_epoch, committed })
    }
    fn decode_flexible<B: Buf>(buf: &mut B, is_flexible: bool) -> Result<Self, DecodeError> {
        ...
        let committed = <bool as KafkaDeserialize>::decode_flexible(buf, is_flexible).map_err(|_| DecodeError::Protocol { message: "failed to decode Committed".into() })?;
        if is_flexible {
            // Tagged fields (skip)
