...
impl ApiResponse for DescribeTransactionsResponse {
    ...
    fn get_api_key() -> ApiKey { ApiKey::new(65) }
    ...
    fn deserialize(version: crate::traits::ApiVersion, buf: &mut Bytes) -> Result<Self, SerializationError> {
        ...
        Ok(Self { throttle_time_ms, transaction_states })
    }
}
...
impl KafkaSerialize for TransactionState {
    fn encode<B: BufMut>(&self, buf: &mut B) -> Result<(), EncodeError> {
        ...
        self.transaction_start_time_ms.encode(buf).map_err(|_| EncodeError::ValueTooLarge { message: "failed to encode TransactionStartTimeMs".into() })?;
    ...
    fn encode_flexible<B: BufMut>(&self, buf: &mut B, is_flexible: bool) -> Result<(), EncodeError> {
        ...
        self.transaction_start_time_ms.encode_flexible(buf, is_flexible).map_err(|_| EncodeError::ValueTooLarge { message: "failed to encode TransactionStartTimeMs".into() })?;
...
impl KafkaDeserialize for TransactionState {
    fn decode<B: Buf>(buf: &mut B) -> Result<Self, DecodeError> {
        ...
        let transaction_start_time_ms = <i64 as KafkaDeserialize>::decode(buf).map_err(|_| DecodeError::Protocol { message: "failed to decode TransactionStartTimeMs".into() })?;
        ...
        Ok(Self { error_code, transactional_id, transaction_state, transaction_timeout_ms, transaction_start_time_ms, producer_id, producer_epoch, topics })
    }
    fn decode_flexible<B: Buf>(buf: &mut B, is_flexible: bool) -> Result<Self, DecodeError> {
        ...
        let transaction_start_time_ms = <i64 as KafkaDeserialize>::decode_flexible(buf, is_flexible).map_err(|_| DecodeError::Protocol { message: "failed to decode TransactionStartTimeMs".into() })?;
