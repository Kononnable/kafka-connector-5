...
impl ApiResponse for WriteShareGroupStateResponse {
    ...
    fn deserialize(version: crate::traits::ApiVersion, buf: &mut Bytes) -> Result<Self, SerializationError> {
        let is_flexible = true;
        let results = <Vec<WriteStateResult> as KafkaDeserialize>::decode_flexible(buf, is_flexible).map_err(|_| SerializationError::Decode("failed to decode Results"))?;
        Ok(Self { results })
    }
}
...
impl KafkaDeserialize for WriteShareGroupStateResponse {
    fn decode<B: Buf>(buf: &mut B) -> Result<Self, DecodeError> {
        let results = <Vec<WriteStateResult> as KafkaDeserialize>::decode(buf).map_err(|_| DecodeError::Protocol { message: "failed to decode Results".into() })?;
        Ok(Self { results })
    }
    fn decode_flexible<B: Buf>(buf: &mut B, is_flexible: bool) -> Result<Self, DecodeError> {
        let results = <Vec<WriteStateResult> as KafkaDeserialize>::decode_flexible(buf, is_flexible).map_err(|_| DecodeError::Protocol { message: "failed to decode Results".into() })?;
        if is_flexible {
            // Tagged fields (skip)
