...
impl ApiResponse for AlterUserScramCredentialsResponse {
    ...
    fn get_api_key() -> ApiKey { ApiKey::new(51) }
    ...
    fn deserialize(version: crate::traits::ApiVersion, buf: &mut Bytes) -> Result<Self, SerializationError> {
        ...
        let results = <Vec<AlterUserScramCredentialsResult> as KafkaDeserialize>::decode_flexible(buf, is_flexible).map_err(|_| SerializationError::Decode("failed to decode Results"))?;
...
impl KafkaDeserialize for AlterUserScramCredentialsResponse {
    fn decode<B: Buf>(buf: &mut B) -> Result<Self, DecodeError> {
        ...
        let results = <Vec<AlterUserScramCredentialsResult> as KafkaDeserialize>::decode(buf).map_err(|_| DecodeError::Protocol { message: "failed to decode Results".into() })?;
        Ok(Self { throttle_time_ms, results })
    }
    fn decode_flexible<B: Buf>(buf: &mut B, is_flexible: bool) -> Result<Self, DecodeError> {
        ...
        let results = <Vec<AlterUserScramCredentialsResult> as KafkaDeserialize>::decode_flexible(buf, is_flexible).map_err(|_| DecodeError::Protocol { message: "failed to decode Results".into() })?;
        if is_flexible {
            // Tagged fields (skip)
...
impl KafkaDeserialize for AlterUserScramCredentialsResult {
    fn decode<B: Buf>(buf: &mut B) -> Result<Self, DecodeError> {
        ...
        Ok(Self { user, error_code, error_message })
    }
    fn decode_flexible<B: Buf>(buf: &mut B, is_flexible: bool) -> Result<Self, DecodeError> {
