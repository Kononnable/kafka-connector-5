...
impl KafkaDeserialize for PartitionData {
    fn decode<B: Buf>(buf: &mut B) -> Result<Self, DecodeError> {
        ...
        tracing::trace!(
            "  [{}] classic decode field `VoteGranted` ({} bytes remaining)",
            stringify!(Self),
            buf.remaining()
        );
        let vote_granted =
    ...
    fn decode_flexible<B: Buf>(
        ...
        tracing::trace!(
            "  [{}] decoding field `VoteGranted` ({} bytes remaining)",
            stringify!(Self),
            buf.remaining()
        );
        let vote_granted = <bool as KafkaDeserialize>::decode_flexible(buf, version, is_flexible)
            .map_err(|_| DecodeError::Protocol {
            message: "failed to decode VoteGranted".into(),
        })?;
        if is_flexible {
