...
impl ApiResponse for GetTelemetrySubscriptionsResponse {
    ...
    fn deserialize(version: ApiVer, buf: &mut Bytes) -> Result<Self, SerializationError> {
        ...
        let accepted_compression_types = KafkaCodec::decode(buf, version, is_flexible)?;
        let push_interval_ms = KafkaCodec::decode(buf, version, is_flexible)?;
        let telemetry_max_bytes = KafkaCodec::decode(buf, version, is_flexible)?;
        let delta_temporality = KafkaCodec::decode(buf, version, is_flexible)?;
        let requested_metrics = KafkaCodec::decode(buf, version, is_flexible)?;
        if is_flexible {
...
impl KafkaCodec for GetTelemetrySubscriptionsResponse {
    fn encode<B: BufMut>(
        &self,
        buf: &mut B,
        version: ApiVer,
