...
impl ApiRequest for DescribeLogDirsRequest {
    ...
    fn deserialize(version: crate::traits::ApiVersion, buf: &mut Bytes) -> Result<Self, SerializationError> {
        let is_flexible = (2) <= version.0;
        let topics = <Option<Vec<DescribableLogDirTopic>> as KafkaDeserialize>::decode_flexible(buf, is_flexible).map_err(|_| SerializationError::Decode("failed to decode Topics"))?;
        Ok(Self { topics })
    }
}
...
impl KafkaDeserialize for DescribeLogDirsRequest {
    fn decode<B: Buf>(buf: &mut B) -> Result<Self, DecodeError> {
        let topics = <Option<Vec<DescribableLogDirTopic>> as KafkaDeserialize>::decode(buf).map_err(|_| DecodeError::Protocol { message: "failed to decode Topics".into() })?;
        Ok(Self { topics })
    }
    fn decode_flexible<B: Buf>(buf: &mut B, is_flexible: bool) -> Result<Self, DecodeError> {
        let topics = if is_flexible {
            ...
            if __present == 0 {
                ...
                Some(<Vec<DescribableLogDirTopic> as KafkaDeserialize>::decode_flexible(buf, true).map_err(|_| DecodeError::Protocol { message: "failed to decode Topics".into() })?)
            }
        } else {
            <Option<Vec<DescribableLogDirTopic>> as KafkaDeserialize>::decode(buf).map_err(|_| DecodeError::Protocol { message: "failed to decode Topics".into() })?
        };
        if is_flexible {
            // Tagged fields (skip)
