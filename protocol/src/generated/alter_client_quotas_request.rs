...
impl KafkaCodec for AlterClientQuotasRequest {
    fn encode<B: BufMut>(
        &self,
        buf: &mut B,
        version: ApiVer,
...
impl KafkaCodec for EntryData {
    ...
    fn decode<B: Buf>(
        ...
        let ops = KafkaCodec::decode(buf, version, is_flexible)?;
        if is_flexible {
            let (_tag_count, _) = decode_unsigned_varint(buf)?;
        }
        Ok(Self { entity, ops })
...
impl KafkaCodec for OpData {
    ...
    fn decode<B: Buf>(
        ...
        let remove = KafkaCodec::decode(buf, version, is_flexible)?;
        if is_flexible {
            let (_tag_count, _) = decode_unsigned_varint(buf)?;
        }
