...
pub fn decode_response_body(api_key: i16, version: i16, body: &[u8]) -> Result<String, String> {
    ...
    let mut buf = Bytes::copy_from_slice(body);
    let result = match api_key {
        0  => ProduceResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        1  => FetchResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        2  => ListOffsetsResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        3  => MetadataResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        8  => OffsetCommitResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        9  => OffsetFetchResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        10 => FindCoordinatorResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        11 => JoinGroupResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        12 => HeartbeatResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        13 => LeaveGroupResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        14 => SyncGroupResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        15 => DescribeGroupsResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        16 => ListGroupsResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        17 => SaslHandshakeResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        18 => ApiVersionsResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        19 => CreateTopicsResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        20 => DeleteTopicsResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        21 => DeleteRecordsResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        22 => InitProducerIdResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        23 => OffsetForLeaderEpochResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        24 => AddPartitionsToTxnResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        25 => AddOffsetsToTxnResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        26 => EndTxnResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        27 => WriteTxnMarkersResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        28 => TxnOffsetCommitResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        29 => DescribeAclsResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        30 => CreateAclsResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        31 => DeleteAclsResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        32 => DescribeConfigsResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        33 => AlterConfigsResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        34 => AlterReplicaLogDirsResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        35 => DescribeLogDirsResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        36 => SaslAuthenticateResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        37 => CreatePartitionsResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        38 => CreateDelegationTokenResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        39 => RenewDelegationTokenResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        40 => ExpireDelegationTokenResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
        41 => DescribeDelegationTokenResponse::deserialize(ver, &mut buf).map(|v| format!("{v:?}")),
